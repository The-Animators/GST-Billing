...
class Productmodel extends CI_Model {
    ...
    public function SaveInvoice() {
        ...
        $this->db->trans_start();
        if($shopid==1){$invoice='MMM';}else{$invoice='I';}
    ...
    public function creditnoteinvoice() {
        ...
        $rtval = FALSE;
        $this->db->trans_start();
        if($shopid==1){$invoice='CrMMM';}else{$invoice='CRD';}
    ...
    public function deleteinvoice() {
		$this->db->query("UPDATE salesmaster_".date('Y')." SET deleted = 1 where invoiceid = '".base64_decode(urldecode($this->uri->segment(2)))."'");
    }
/*
    public function deleteinvoice() {
    ...
    public function SavePurchase() {
        $shopid = $this->session->userdata("shopid");
        $this->db->trans_start();
        if($shopid==1){$pinvoice='Pur';}else{$pinvoice='P';}
    ...
    function Savedebitnote() {
        $shopid = $this->session->userdata("shopid");
        $this->db->trans_start();
         if($shopid==1){$dinvoice='RPur';}else{$dinvoice='D';}
